...
with st.sidebar:
    ...
    # Inject CSS to hide the Welcome nav button when NOT on Welcome page
    _on_welcome = st.session_state.nav_page == "🏠 Welcome"
    st.markdown(f"""
    <style>
    /* Hide the Welcome sidebar button unless it's active */
    {"" if _on_welcome else """
...
# ══════════════════════════════════════════
# MAIN — tiny home button on non-home pages
# ══════════════════════════════════════════
page = st.session_state.nav_page

if page != "🏠 home":
    # Small home button + page title in one row
    ...
    with _tcol:
        ...
        st.markdown(
            f"<div style='font-family:Rajdhani,sans-serif;font-weight:700;font-size:14px;"
            f"color:{tdim};letter-spacing:1px;padding-top:6px;'>{page}</div>",
            unsafe_allow_html=True)
...
if page == "🏠 Welcome":
    ...
    _avatar_colors = ["#e05252","#7ecfff","#52e08a","#f5a623","#a855f7","#ec4899","#06b6d4"]
    avatar_color   = _avatar_colors[sum(ord(c) for c in username) % len(_avatar_colors)]

    st.markdown(f"""
    <style>
    @keyframes hero-in  {{ from{{opacity:0;transform:translateY(-18px)}} to{{opacity:1;transform:translateY(0)}} }}
    @keyframes avatar-pop {{ 0%{{transform:scale(0.6);opacity:0}} 70%{{transform:scale(1.08);opacity:1}} 100%{{transform:scale(1);opacity:1}} }}
    @keyframes shimmer-l {{ 0%{{background-position:-400px 0}} 100%{{background-position:400px 0}} }}
    @keyframes ci1 {{ from{{opacity:0;transform:translateY(18px)}} to{{opacity:1;transform:translateY(0)}} }}
    @keyframes ci2 {{ 0%{{opacity:0;transform:translateY(18px)}} 25%{{opacity:0}} 100%{{opacity:1;transform:translateY(0)}} }}
    @keyframes ci3 {{ 0%{{opacity:0;transform:translateY(18px)}} 45%{{opacity:0}} 100%{{opacity:1;transform:translateY(0)}} }}
    @keyframes ci4 {{ 0%{{opacity:0;transform:translateY(18px)}} 60%{{opacity:0}} 100%{{opacity:1;transform:translateY(0)}} }}

    .wh-hero {{
        background:linear-gradient(135deg,{bg2} 0%,{bg3} 100%);
        border:1px solid {bord}; border-top:3px solid {avatar_color};
        border-radius:16px; padding:32px 36px 28px 36px;
        display:flex; align-items:center; gap:26px;
        animation:hero-in 0.5s cubic-bezier(.22,.68,0,1.1) both;
        margin-bottom:18px; position:relative; overflow:hidden;
    }}
    .wh-hero::after {{
        content:''; position:absolute; top:-40px; right:-40px;
        width:200px; height:200px;
        background:radial-gradient(circle,{avatar_color}18 0%,transparent 70%);
        pointer-events:none;
    }}
    .wh-avatar {{
        flex-shrink:0; width:68px; height:68px; border-radius:50%;
        background:linear-gradient(135deg,{avatar_color}cc,{avatar_color}44);
        border:2px solid {avatar_color};
        display:flex; align-items:center; justify-content:center;
        font-family:'Orbitron',sans-serif; font-size:20px; font-weight:900; color:white;
        box-shadow:0 0 22px {avatar_color}44;
        animation:avatar-pop 0.5s 0.15s ease both;
    }}
    .wh-text {{ flex:1; min-width:0; }}
    .wh-greeting {{ font-family:'Share Tech Mono',monospace; font-size:10px; color:{tdim}; letter-spacing:2px; margin-bottom:3px; }}
    .wh-name {{ font-family:'Orbitron',sans-serif; font-size:clamp(1.2rem,2.2vw,1.7rem); font-weight:900; letter-spacing:3px; margin-bottom:5px; }}
    .wh-shimmer {{ height:2px; width:100%; max-width:280px;
        background:linear-gradient(90deg,transparent,{avatar_color},{avatar_color},transparent);
        background-size:400px 2px; animation:shimmer-l 2.5s linear infinite; margin-bottom:7px; border-radius:2px; }}
    .wh-meta {{ font-family:'Share Tech Mono',monospace; font-size:11px; color:{tdim}; }}
    .wh-badge {{ position:absolute; top:14px; right:18px;
        background:{avatar_color}22; border:1px solid {avatar_color}66; border-radius:20px;
        padding:3px 11px; font-family:'Share Tech Mono',monospace; font-size:10px; color:{avatar_color}; letter-spacing:1px; }}

    .wh-stats {{ display:grid; grid-template-columns:repeat(4,1fr); gap:11px; margin-bottom:18px; animation:ci1 0.5s 0.2s ease both; }}
    .wh-stat {{ background:{bg2}; border:1px solid {bord}; border-radius:10px; padding:15px 12px; text-align:center; transition:border-color .2s,transform .15s; }}
    .wh-stat:hover {{ border-color:{accent}; transform:translateY(-2px); }}
    .wh-stat-icon {{ font-size:18px; margin-bottom:5px; }}
    .wh-stat-val  {{ font-family:'Orbitron',sans-serif; font-size:16px; font-weight:900; color:{tblue}; margin-bottom:2px; }}
    .wh-stat-lbl  {{ font-family:'Rajdhani',sans-serif; font-size:9px; color:{tdim2}; text-transform:uppercase; letter-spacing:1.5px; }}

    .wh-slbl {{ font-family:'Share Tech Mono',monospace; font-size:9px; color:{tdim2}; letter-spacing:3px; margin:6px 0 10px 0; }}

    .wh-steps {{ display:grid; grid-template-columns:repeat(4,1fr); gap:11px; margin-bottom:18px; }}
    .wh-step {{ background:{bg2}; border:1px solid {bord}; border-radius:12px; padding:18px 14px 16px; position:relative; overflow:hidden; transition:box-shadow .2s,border-color .2s; }}
    .wh-step:hover {{ box-shadow:0 6px 22px rgba(0,0,0,.22); }}
    .wh-s1 {{ border-left:3px solid {accent}; animation:ci1 0.5s 0.22s ease both; }}
    .wh-s2 {{ border-left:3px solid {tblue}; animation:ci2 0.7s ease both; }}
    .wh-s3 {{ border-left:3px solid #52e08a; animation:ci3 0.7s ease both; }}
    .wh-s4 {{ border-left:3px solid #f5a623; animation:ci4 0.7s ease both; }}
    .wh-snum {{ position:absolute; top:8px; right:10px; font-family:'Orbitron',sans-serif; font-size:22px; font-weight:900; color:{bord}; line-height:1; }}
    .wh-sicon {{ font-size:20px; margin-bottom:8px; }}
    .wh-stitle {{ font-family:'Rajdhani',sans-serif; font-weight:700; font-size:12px; letter-spacing:1px; margin-bottom:6px; }}
    .wh-sdesc  {{ font-family:'Rajdhani',sans-serif; font-size:11px; color:{tdim}; line-height:1.65; }}

    .wh-caps {{ background:{bg2}; border:1px solid {bord}; border-left:4px solid {accent}; border-radius:10px; padding:16px 20px; animation:ci2 0.6s ease both; }}
    </style>

    <!-- HERO -->
    <div class="wh-hero">
      <div class="wh-avatar">{user_initials}</div>
      <div class="wh-text">
        <div class="wh-greeting">{greeting_icon} &nbsp;{greeting_word.upper()}</div>
        <div class="wh-name">{username.upper()}</div>
        <div class="wh-shimmer"></div>
        <div class="wh-meta">{today} &nbsp;·&nbsp; Session {st.session_state.run_id}</div>
      </div>
      <div class="wh-badge">🛡️ OPERATOR</div>
    </div>

    <!-- STATS -->
    <div class="wh-stats">
      <div class="wh-stat"><div class="wh-stat-icon">📂</div><div class="wh-stat-val">{records_count}</div><div class="wh-stat-lbl">Videos</div></div>
      <div class="wh-stat"><div class="wh-stat-icon">🔥</div><div class="wh-stat-val">4</div><div class="wh-stat-lbl">CAM Methods</div></div>
      <div class="wh-stat"><div class="wh-stat-icon">🥊</div><div class="wh-stat-val" style="color:#52e08a;">ON</div><div class="wh-stat-lbl">ByteTrack</div></div>
      <div class="wh-stat"><div class="wh-stat-icon">🧠</div><div class="wh-stat-val" style="color:#f5a623;font-size:12px;">R3D-18</div><div class="wh-stat-lbl">Backbone</div></div>
    </div>

    <!-- HOW TO USE — 4 STEPS -->
    <div class="wh-slbl">── HOW TO USE VISIONGUARD ──</div>
    <div class="wh-steps">

      <div class="wh-step wh-s1">
        <div class="wh-snum">01</div>
        <div class="wh-sicon">📤</div>
        <div class="wh-stitle" style="color:{accent};">UPLOAD</div>
        <div class="wh-sdesc">
          Go to <b>Upload Manager</b> in the sidebar.<br><br>
          Upload your GradCAM output folder with:<br>
          · original.mp4<br>
          · gradcam / gradcampp / smooth_gradcampp .mp4<br>
          · layercam.mp4 &amp; combined.mp4<br>
          · grid PNGs + pred.txt
        </div>
      </div>

      <div class="wh-step wh-s2">
        <div class="wh-snum">02</div>
        <div class="wh-sicon">🔍</div>
        <div class="wh-stitle" style="color:{tblue};">ANALYZE</div>
        <div class="wh-sdesc">
          Open <b>Video Explorer</b>.<br><br>
          Choose dataset → class → folder from the dropdowns.<br><br>
          Press <b>Analyze Folder</b> to load predictions, video frames, and compute the fight probability timeline.
        </div>
      </div>

      <div class="wh-step wh-s3">
        <div class="wh-snum">03</div>
        <div class="wh-sicon">🔥</div>
        <div class="wh-stitle" style="color:#52e08a;">GRADCAM VIEWER</div>
        <div class="wh-sdesc">
          Open <b>GradCAM Viewer</b>.<br><br>
          Watch all 6 overlay videos side by side:<br>
          · GradCAM<br>
          · GradCAM++<br>
          · Smooth GradCAM++<br>
          · LayerCAM<br>
      ...
      <div class="wh-step wh-s4">
        <div class="wh-snum">04</div>
        <div class="wh-sicon">🖼️</div>
        <div class="wh-stitle" style="color:#f5a623;">GRID VIEWER</div>
        <div class="wh-sdesc">
          Open <b>Grid Viewer</b>.<br><br>
          See frame-by-frame grids for every CAM method to inspect which frames triggered detection.<br><br>
          Check <b>Timeline</b> for the fight onset probability plot.
        </div>
      </div>
    ...
    <!-- CAPABILITIES -->
    <div class="wh-caps">
      <div style="font-family:'Orbitron',sans-serif;font-size:10px;font-weight:900;color:{accent};letter-spacing:3px;margin-bottom:10px;">🛡️ SYSTEM CAPABILITIES</div>
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;font-family:'Share Tech Mono',monospace;font-size:11px;">
        <div><div style="color:{tdim2};font-size:9px;letter-spacing:2px;margin-bottom:5px;">DETECTION</div>
          <div style="line-height:1.85;">Real-time violence detection<br>Fight onset localization<br>Confidence scoring<br>Multi-window analysis</div></div>
        <div><div style="color:{tdim2};font-size:9px;letter-spacing:2px;margin-bottom:5px;">VISUALIZATION</div>
          <div style="line-height:1.85;">GradCAM heatmaps<br>GradCAM++ overlays<br>Smooth GradCAM++<br>LayerCAM · Combined</div></div>
        <div><div style="color:{tdim2};font-size:9px;letter-spacing:2px;margin-bottom:5px;">ANALYTICS</div>
          <div style="line-height:1.85;">ByteTrack fighter IDs<br>Confusion matrix<br>FP/FN browser<br>PDF incident reports</div></div>
      </div>
    </div>
    """, unsafe_allow_html=True)

    # ── Quick-launch buttons — only on Welcome ──
    st.markdown(f"""<div style="font-family:'Share Tech Mono',monospace;font-size:9px;
        color:{tdim2};letter-spacing:3px;margin:20px 0 10px 0;">── QUICK LAUNCH ──</div>""",
        unsafe_allow_html=True)
    _ql_cols = st.columns(6, gap="small")
    _ql_items = [
        ("📁 Explorer",  "📁 Video Explorer"),
        ("📤 Upload",    "📤 Upload Manager"),
        ("🔥 GradCAM",   "🔥 GradCAM Viewer"),
        ("🖼️ Grid",      "🖼️ Grid Viewer"),
        ("📊 Stats",     "📊 Dataset Stats"),
        ("⚙️ Settings",  "⚙️ Settings"),
    ]
    for _c, (_lbl, _tgt) in zip(_ql_cols, _ql_items):
        with _c:
    ...
    _, _lo_col, _ = st.columns([3, 1, 3])
    with _lo_col:
        if st.button("🚪 Logout", use_container_width=True, key="welcome_logout"):
            for k in list(st.session_state.keys()): del st.session_state[k]
            st.rerun()
