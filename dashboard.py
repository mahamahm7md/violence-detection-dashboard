...
def confidence_to_status(conf_str: str) -> str:
    """Convert confidence from pred.txt directly to status — no guessing needed."""
    try:
        c = float(conf_str)
...
def pred_label_to_status(pred_label: str) -> str:
    """Use the actual model prediction label as the ground truth status."""
...
def scores_from_pred(pred: dict, n_frames: int, fps: float):
    """
    Build a realistic score array from pred.txt data.
    Uses the real confidence and onset_frame from pred.txt.
    This means status/analytics match the actual model output.
    """
    conf = 0.5
    ...
    if is_fight:
        # Ramp up from onset_frame to end with the real confidence
        ...
        # NonFight: low scores, below threshold
...
def make_timeline_plot(scores, fps, pred=None):
    ...
    # color the line red if fight, green if not
    is_fight = False
    if pred:
...
def clear_all_uploads():
    """Delete everything in the uploads folder."""
...
with tabs[0]:
    ...
    if st.button("🔍 Analyze this folder", type="primary",
        ...
        # Build scores FROM pred.txt — so status is always accurate
    ...
    if st.session_state.active_folder_name:
        ...
        # Quick Analysis — driven by pred.txt
...
with tabs[1]:
    ...
    if not records:
        ...
        with fc1: sq   = st.text_input("Search folder name", placeholder="e.g. fi1")
...
with tabs[2]:
    ...
    l_ds,l_cls,l_fn = folder_sel("cl",lc)
    r_ds,r_cls,r_fn = folder_sel("cr",rc)

    if st.button("⚖️ Compare",type="primary",disabled=(not l_fn or not r_fn)):
...
with tabs[3]:
    ...
    with cA:
        ...
        if st.button("▶️ Run Preview",type="primary",disabled=not bool(vpath)):
            try:
...
with tabs[4]:
    ...
    if st.button("🧱 Render Wall",disabled=(not cams or not all(bool(x) for x in cams))):
...
with tabs[5]:
    ...
    if upload_mode=="📁 Single folder (select files)":
        ...
        with uc3: fn_inp = st.text_input("Folder name",placeholder="e.g. fi1_xvid")
        ...
        if st.button("💾 Save",type="primary",disabled=(not up_files or not fn_inp.strip())):
...
```""")
        ...
        if st.button("📦 Extract ZIP",type="primary",disabled=(not zf)):
...
# ══════════════════════════════════════════
# TAB 7 — ANALYTICS (follows active folder)
# ══════════════════════════════════════════
with tabs[6]:
...
with tabs[9]:
    ...
    if not records:
        ...
        with t1c: new_thr_v=st.slider("Violence threshold",   0.10,0.99,CFG.THRESH_VIOLENCE,   0.01)
        with t2c: new_thr_s=st.slider("Suspicious threshold", 0.10,0.99,CFG.THRESH_SUSPICIOUS, 0.01)
        df=pd.DataFrame(records)
...
# ══════════════════════════════════════════
# TAB 12 — INCIDENT REPORT (follows active folder)
# ══════════════════════════════════════════
with tabs[11]:
